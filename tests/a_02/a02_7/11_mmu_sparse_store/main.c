// a02_7 / 11_mmu_sparse_store — setup_pt()
//
...
// → 循环里 8 条 sw 顺序访这 8 页, direct-mapped 下每条都挤掉上一条 → 条条 TLB miss
//   → 每条都走 mmu_walker_helper_store → mmu_walk 两级重走 + store_helper。
// PTE.W/D 起步置 1, store 走通且 walker 不卡 hw-managed D。
