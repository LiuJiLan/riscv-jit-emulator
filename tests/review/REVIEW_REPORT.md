...
a02_7 16 fixture 同条件 release median × 5 跑批对比 (b_04_02 收尾态; 跟 b_03
T_收尾相比仅 blocksize_64 在软边界对齐修复后翻倍, 其他持平):

| Fixture                | 解释器 (a_03 T6.2) | JIT (b_04_02) |     加速比 |
| ---------------------- | -----------------: | ------------: | ---------: |
| **取指 / 纯 ALU**      |                    |               |            |
...
| 16_blocksize_64        |              201.2 |       3239.7  | **16.10x** |

统计 (b_04_02 收尾态):

1. 算术平均加速比 ~3.7x (b_04_02 软边界对齐后微涨 ~0.3, 详 §4.2 反序段)
2. 几何平均加速比 ~2.4x (抗极值漂移, 更接近 "典型加速")
3. 最大加速比 16.10x (16_blocksize_64, 大块 + 寄存器映射 + 软边界字面对齐三因
   叠加, 见 §5.1 + §4.2 反序段)
4. 最小加速比 0.81x (03_csr_heavy, 块大小敏感性导致退化, 见 §5.4)

### 4.2 加速曲线: 块大小是主导因子

块大小 sweep 的曲线最清晰显示 JIT 加速跟 guest 平均块大小的关系 (b_04_02
收尾态):
...
b_04_02 软边界字面对齐 trail (详 REVIEW.md b_04_02 段): b_03 收尾态曾测得
`16_blocksize_64` 加速比 7.80x (反序于 15_blocksize_32 10.53x), 原归因 "大块
emit 后 host I-cache 容量边界". 实际定位为 misdiagnosis: JIT translator 软边界
`while (i < BLOCK_INST_LIMIT - 1u)` 比解释器少 1 条, 撞 fixture 块大小 64 inst
时把 fixture 一块强制切成 (63 + 1) 两段, 第二段单 inst dispatch overhead 几乎
翻倍. b_04_02 改 `while (i < BLOCK_INST_LIMIT)` + `ir_buf[BLOCK_INST_LIMIT + 1]`
后 fixture 一段跑完, 16_blocksize_64 = 16.10x, 32 → 64 加速曲线单调上升, 反序
消失.

实测一致性: 从块大小曲线反推 dispatcher round-trip 约 12-15ns / 块 (`a02_7/12`
跟 `a02_7/16` 两端外推, b_04_02 后 16_blocksize_64 数据点更接近线性外推预测).
后续 §5.3 fence.i 实验也独立验证此数字量级.
...
1. JIT 全套 a02_7 加速: 几何均值 ~2.4x, 算术均值 ~3.7x (b_04_02 软边界对齐后)
2. 大块场景: 最高 16.10x (a02_7/16_blocksize_64, b_04_02 软边界对齐修了原 7.80x
   反序; 详 §4.2 反序段 + REVIEW.md b_04_02 段)
